...
void add() {
	...
	for(; li <= e; li++) {
		...
		if(cd? tr[cn][cc][cd] == ss[e] : tr[cn].count(ss[e])) {
			if(cd++ == 0) cc = ss[e];
		...
		if(cd) {
			...
			int g = cn? e - cd : li + 1;
			cn = suf[cn];
			while(tr[cn].count(cc = ss[g]) && g + tr[cn][ss[g]].len() <= e) {
				g += tr[cn][cc].len();
				cn = tr[cn][cc].n;
			}
			cd = e - g;
...
void dfs(int u, ll ct) {
	...
	if(st[u].size() >= k) {
		for(pii e : st[u])
			all[e.fst] += ll(ct - tr[u].empty()) * ll(e.snd);
	}
}
